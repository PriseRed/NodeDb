...
const userScheme = new Schema({name: String, surname: String, age: Number}, {versionKey: false});
...
app.post("/api/users", jsonParser, async (req, res) =>{
    ...
    const user = new User({name: userName, surname: userSurname, age: userAge});
    // сохраняем в бд
    await user.save();
    res.send(user);
});
...
app.put("/api/users", jsonParser, async (req, res)=>{
    ...
    const userSurname = req.body.surname;
    const userAge = req.body.age;
    const newUser = {age: userAge, surname: userSurname, name: userName};
